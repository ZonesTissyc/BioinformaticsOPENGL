...
#pragma region openGL 基础设置
    ...
    // ------------------------------------------------------------------
    Camera camera(glm::vec3(0.10f, 5.43f, -4.3f));
    ...
    std::vector<PointLight> pointLights = {
        {{-10.0f, 10.0f, -10.0f}, {1.0f, 0.0f, 0.0f}, 1.0f, 1.0f, 0.09f, 0.032f},  // 红色光源
        {{10.0f, 10.0f, -10.0f}, {0.0f, 1.0f, 0.0f}, 1.0f, 1.0f, 0.09f, 0.032f},  // 绿色光源
        {{-10.0f, 10.0f, 10.0f}, {0.0f, 0.0f, 1.0f}, 1.0f, 1.0f, 0.09f, 0.032f},  // 蓝色光源
        {{10.0f, 10.0f, 10.0f}, {1.0f, 1.0f, 1.0f}, 1.0f, 1.0f, 0.09f, 0.032f}   // 白色光源
    };
...
#pragma endregion
    ...
    while (window.noClose()) {
        ...
        // 绘制骨骼动画模型 - 使用anim_model着色器
        Renderer::BeginScene(camera, projMat, shader1);
        player2.Update(deltaTime);
        player2.Draw(shader1);
        Renderer::EndScene();

        // 绘制静态模型 - 使用Blinn_phong光照明模型
        Renderer::BeginScene(camera, projMat, shaderModel);
        
        // 设置材质属性
        shaderModel.setVec3("material.ambient", 0.1f, 0.1f, 0.1f);
        shaderModel.setVec3("material.specular", 1.0f, 1.0f, 1.0f);
        ...
        for (int i = 0; i < numLights; i++) {
            ...
            shaderModel.setVec3(lightPrefix + ".position", pointLights[i].position);
            shaderModel.setVec3(lightPrefix + ".color", pointLights[i].color);
            shaderModel.setFloat(lightPrefix + ".intensity", pointLights[i].intensity);
            shaderModel.setFloat(lightPrefix + ".constant", pointLights[i].constant);
            shaderModel.setFloat(lightPrefix + ".linear", pointLights[i].linear);
            shaderModel.setFloat(lightPrefix + ".quadratic", pointLights[i].quadratic);
        }
        
        // 提交模型渲染
        Renderer::Submit(shaderModel, modelStatic.get(), transmatStatic.getModelMatrix());
        Renderer::EndScene();
