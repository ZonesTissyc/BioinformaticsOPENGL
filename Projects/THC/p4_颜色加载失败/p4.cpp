...
#pragma region 主循环
     Camera camera(glm::vec3(0.00f, 0.0f, 3.0f));
     InputController inputcontronller(camera, 10.0);
    //窗口未被标记为关闭时持续运行
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 处理输入
        processInput(window);
        inputcontronller.processKeyboardInput(window, deltatime);
        inputcontronller.processMouseInput(window);
     ...
     #pragma region 物体的渲染
        ...
        glm::mat4 view = camera.getView(); // 用camera类
